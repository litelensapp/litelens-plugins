      ...
      <ScrollArea className="shrink-0">
        <div className="grid grid-cols-[160px_1fr] items-start gap-y-3 p-4">
          ...
          <div>
            ...
            <span className="text-muted-foreground font-mono">({data.UpdatedAt})</span>
          </div>
        </div>
      </ScrollArea>

      <div ref={contentRef} className="flex min-h-0 flex-1 flex-col">
        <div className="bg-muted/50 flex shrink-0 items-center justify-between gap-2 border-t px-4 py-2 text-xs font-semibold uppercase tracking-wide">
          <span>Values</span>
          <FullTextSearchInput
...
const HelmReleaseResourcesTab: FC<{ resources: HelmReleaseResource[] }> = ({ resources }) => {
  ...
  if (resources.length === 0) {
    return (
      <p className="text-muted-foreground p-4 text-xs">No resources found for this release.</p>
    );
  }
              ...
              <table className="w-full table-fixed text-xs">
                <thead>
                  <tr className="bg-muted/50 border-b">
                    <th className="text-muted-foreground w-3/4 px-3 py-2 text-left font-medium">
                      Name
                    </th>
                    <th className="text-muted-foreground w-1/4 px-3 py-2 text-left font-medium">
                      Namespace
                    </th>
                  </tr>
                </thead>
...
> = ({ releaseName, namespace, open, onClose, onDataChange }) => {
