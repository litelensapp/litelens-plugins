...
export const HelmChartVersionSelectDropdown: FC<HelmChartVersionSelectDropdownProps> = ({
  ...
  <DropdownMenu>
    <DropdownMenuTrigger
      ...
      className={cn(
        "group/button border-border bg-background hover:bg-muted hover:text-foreground aria-expanded:bg-muted aria-expanded:text-foreground focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/75 inline-flex h-7 cursor-pointer items-center justify-between gap-1.5 whitespace-nowrap rounded-[min(var(--radius-md),12px)] border px-2.5 font-mono text-[0.8rem] font-medium transition-all disabled:pointer-events-none disabled:opacity-50",
        className
      )}
    >
      {isLoading ? (
